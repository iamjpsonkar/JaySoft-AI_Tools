...
| 10 | `session` | sub: `save <name>` (`--task/-t`, `--skill/-s`, `--status`, `--step/-p`, `--finding/-f`, `--no-context`, `--repo/-r`)<br>· `load <name>` (by identifier), `list` (`--skill/-s`, `--status`, `--limit/-n` 20)<br>· `show <name>`, `resume <name>` (newest by default), `rm <name>`, `prune` | `jsat/_cli_session.py:43,83,104,152,167,235`<br>→ `jsat/_sessions.py` | CLI (resumable skill sessions) |
