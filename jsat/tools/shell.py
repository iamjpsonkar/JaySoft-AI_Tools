...
def launch_ai_with_jsat_tools(
    ...
    if ai in ("claude", "claude-cli") and shutil.which("claude"):
        ...
        jsat_context = (
            ...
            "  jsat__get_function       — look up any function (params, return type, complexity)\n"
            "  jsat__get_class          — look up any class (bases, method count, file)\n"
            "  jsat__list_services      — list all indexed services\n"
            "  jsat__list_endpoints     — list all API endpoints\n"
            "  jsat__trace_call_chain   — trace a full call chain from a symbol\n"
            "  jsat__get_index_status   — graph stats (nodes, edges)\n"
            "  jsat__index_repo         — rebuild the codebase graph\n"
            "\n"
            "Impact & safety:\n"
            "  jsat__blast_radius       — trace downstream impact of any file/symbol change\n"
            "  jsat__security_review    — OWASP scan, secret detection, auth gap analysis\n"
            ...
            "  jsat__knowledge_query    — search ADRs, runbooks, architectural decisions\n"
            "  jsat__investigate_incident — score commits as root-cause hypotheses\n"
            "  jsat__generate_runbook   — generate an incident runbook for a service\n"
            "\n"
            "Prompt & token tools:\n"
            ...
            "\n"
            "Planning:\n"
            "  jsat__ithinking_plan     — structured planning before acting (phases 0-4)\n"
            "  jsat__ithinking_reflect  — record what was done (phase 6 log)\n"
            "\n"
            "You also have /jsat-* slash commands — type / to see all 28.\n"
            "Run 'jsat connect claude' to install them if not already done."
        )
