...
from ._cli_common import _jsat, _read_json, _write_json, app, console, err, skills_app
from ._cli_connect import _remove_jsat_block, _remove_toml_mcp_server
...
def cmd_disconnect(
    ...
    if tool_lower in ("codex", "ollama", "all"):
        if tool_lower == "all" or scope == "all":
            ...
            # `jsat connect codex` is global-only now. Keep accepting the old default
            # but make the useful cleanup happen.
            scopes = ["global"]
        else:
            scopes = [scope]
        for s in scopes:
            base = Path.cwd() if s == "project" else Path.home()
            toml_path = base / ".codex" / "config.toml"
            if _remove_toml_mcp_server(toml_path, "jsat"):
                console.print(
                    f"[green]✓[/] Removed JSAT from [bold]Codex[/] ({toml_path})"
                )
                removed_any = True

            # Backward compatibility: older JSAT wrote a JSON Codex config.
            removed_any |= _remove_from_standard("Codex", base / ".codex" / "config.json")

            if not keep_skills and s == "project":
                # Clean files generated by older/preview Codex integrations only when
                # project scope is explicitly included.
                        ...
                        if p.is_available():
                            if hasattr(p, "configure"):
                                with contextlib.suppress(Exception):
                                    p.configure(repo_dir=str(repo_path), stateful=False)
                            return p
                    except Exception:
                        pass
                    return None
