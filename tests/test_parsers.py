...
def test_js_function(tmp_path):
    ...
    fns = _nodes(JavaScriptParser().parse(f, tmp_path), "Function")
    names = [n["properties"]["name"] for n in fns]
...
def test_java_file_node(tmp_path):
    pytest.importorskip(
        "tree_sitter_java",
        reason="tree-sitter-java not installed (pip install 'jsat[standard]')",
    )
...
def test_java_class(tmp_path):
    ...
    f = tmp_path / "Foo.java"
    f.write_text("public class Foo { public void bar(){} }\n")
    from jsat._parsers.java import JavaParser
...
def test_ruby_method(tmp_path):
    ...
    from jsat._parsers.ruby import RubyParser
    nodes = RubyParser().parse(f, tmp_path).nodes
    names = [n["properties"]["name"] for n in nodes if n["label"] == "Function"]
    assert "greet" in names
...
def test_rust_function(tmp_path):
    ...
    from jsat._parsers.rust import RustParser
    nodes = RustParser().parse(f, tmp_path).nodes
    names = [n["properties"]["name"] for n in nodes if n["label"] == "Function"]
    assert "add" in names
